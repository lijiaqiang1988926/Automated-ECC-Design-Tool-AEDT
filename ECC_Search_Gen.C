...
int main(int argc, char *argv[]){
  ...
  float Stop_time = atof(argv[5]);    // stopping time
  ...
  if(Mode == 0){
    ...
    int *Column_Weight = new int[Length_Code]; //Column_Weight
    ...
    // read data
      ...
      {cout<<"----Read Data Fail----"<<endl; break;}
    ...
    while(cnt){
      ...
      // read data
      if(tmp[0]=='*')
      cout<<"----Read Column_Weight Successful----"<<endl;
      else
      {cout<<"----Read Column_Weight Fail----"<<endl; break;}
      // read column_weight
      for (int i=0;i<Length_Code;i++){
        fin_1>> *(Column_Weight+i);
      }
    }
  ...
  // --------------------------------
    ...
    for(int i=0;i<(Length_D+Length_R);i++){
      ...
      if(flag_dp==0){
        ...
        fout<<"if(Counter(array[i"<<i<<"],Length_R) > "<<*(Column_Weight+i)<<") continue;"<<endl;
      ...
      if(flag_dp==1){
        ...
        fout<<"if(Counter(i"<<i<<",Length_R) > "<<*(Column_Weight+i)<<") continue;"<<endl;
